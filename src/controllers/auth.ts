...
export const oAuth: RequestHandler = async (req, res, next) => {
    try {
        const { user:{name, email, picture} } = req.body;
        console.log(email, name, picture)
        if (!email || !name) {
            return next(createError('Please Provide all the fields', 400));
        }
        const userExists = await USER.findOne({ email });
        if (userExists) {
            let token = userExists.tokens[0].token;
            if (!token) token = await userExists.generateAuthToken();
